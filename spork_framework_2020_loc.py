...
def multi_case_algorithm_2020(zdrlev, kdplev, REFlev, REFlev1, big_storm, zero_z_trigger, storm_to_track, year, month, day, hour, start_min, duration, calibration, station, Bunkers_m,localfolder, track_dis=10, GR_mins=5.0):
    ...
    #Local file option:
        ...
        try:
            print(str(folder)+'/'+(str(radar_file)[2:-1]))
            radar1 = pyart.io.nexrad_archive.read_nexrad_archive(str(folder)+'/'+(str(radar_file)[2:-1]))
        except:
...
#             print(rlons_2d)
#             print(rlats_2d)
            [az_masked, shear_maxes1, shear_maxes15, shear_maxes2, shear_maxes25, shear_lats1, shear_lats15, shear_lats2, shear_lats25, shear_lons1, shear_lons15, shear_lons2, shear_lons25, azim_save] = get_rotation(np.asarray(VEL), REFall, rlons_2d, rlats_2d, bin_size=7)
...
#             ZDR_sum_stuff = ZDR_new_sum
        ...
        # zdrp = ax.pcolormesh(rlons_2d, rlats_2d, ZDRmasked1, cmap=plt.cm.nipy_spectral, vmin = -2, vmax = 6)
        #print(np.nanmean(ZDRmasked1))
        #print('fail       loooooooook here')
        ...
        if len(max_lons_c) > 0:
            ...
            for i in enumerate(max_lons_c):
                ...
                try:
                    ...
                    #ZDR column volume section
                    all_dpths = np.copy(ZDR_sum_stuff[mask_con_col])
                    print(all_dpths, 'all_dpths')
                    all_dpths = all_dpths - 4
                    all_dpths[all_dpths < 0] = 0
                    dpth_sum = np.nansum(all_dpths)
                    print(dpth_sum, 'dpth_sum')
                    col_con_vol.append(dpth_sum*500*500*250)
...
#                 rawangle_k = rawangle_k*(-1)
            ...
            for storm in enumerate(max_lons_c):
                ...
                if matching_ind_col.shape[0] > 0:
                    ...
                    p_col_vols.append((col_con_vol[matching_ind_col[0]]))
                else:
                    p_hail_areas.append((0))
...
#                    mm_7 = ax.scatter(rot25_con_lon, rot25_con_lat, marker = 'v', s = 100, color = 'green', zorder = 20, transform=ccrs.PlateCarree(), label="7km meso")
        ...
        o_sp_angle = np.asarray(o_sp_angle)
        ...
        try:
            ...
            legend_sep = o_sp_angle[tracking_ind]
        except:
            legend_zdr=0
            legend_hail=0
            legend_col=0
        ...
        try:
            ...
            print('oops storm')
        alt_levs = [1000, 2000]
        plt.savefig('Machine_Learning/SPORK_STAMP'+station+str(time_start.year)+str(time_start.month)+str(day)+str(hour)+str(minute)+'.png')
        print('Figure Saved')

#Get postage stamps for the CNN project
        try:
            #postagestorm = new_cells.xs(storm_to_track, level=1)
            postlon = max_lons_c[tracking_ind]
            postlat = max_lats_c[tracking_ind]

            postsize = 70

            lond = np.abs(rlons_2d-postlon)
            latd = np.abs(rlats_2d-postlat)
            ...
            #print(np.where(lond==np.min(lond)))
            #print(np.where(latd==np.min(latd)))

            REFpost = REFall[:,xminlat-postsize:xminlat+postsize, yminlon-postsize:yminlon+postsize]
            ZDRpost = ZDRall[:,xminlat-postsize:xminlat+postsize, yminlon-postsize:yminlon+postsize]
            KDPpost = KDPall[:,xminlat-postsize:xminlat+postsize, yminlon-postsize:yminlon+postsize]
            CCpost = CCall[:,xminlat-postsize:xminlat+postsize, yminlon-postsize:yminlon+postsize]
            NZDRpost = NORM_ZDR[xminlat-postsize:xminlat+postsize, yminlon-postsize:yminlon+postsize]
            try:
                ROTpost = az_masked[:,xminlat-postsize:xminlat+postsize, yminlon-postsize:yminlon+postsize]/493
            except:
                ROTpost = CCpost
                ROTpost[:,:,:] = 0.0
            ZDR_D_post = ZDR_sum_stuff[xminlat-postsize:xminlat+postsize, yminlon-postsize:yminlon+postsize]
            try:
                azim_post = azim_save[xminlat-postsize:xminlat+postsize, yminlon-postsize:yminlon+postsize]
            except:
                azim_post = CCpost[0,:,:]
                azim_post[:,:] = 0.0
            try:
                vel_ARR1 = np.asarray(VEL)
                VELpost = vel_ARR1[:,xminlat-postsize:xminlat+postsize, yminlon-postsize:yminlon+postsize]
            except:
                VELpost = CCpost
                VELpost[:,:,:] = 0.0
            #print(np.where(mask_post==True))
#             try:
#                 zdr_mask_stamps.append(mask_post[xminlat-postsize:xminlat+postsize, yminlon-postsize:yminlon+postsize])
#                 print(xminlat)
#                 print(yminlon)
#                 print(np.where(mask_post[xminlat-postsize:xminlat+postsize, yminlon-postsize:yminlon+postsize]==True))
#             except:
#                 print('No MASK')
#                 zdr_mask_stamps.append(np.zeros((REFpost.shape[0], REFpost.shape[1]), dtype=bool))
            ...
            REF_stamps.append(REFpost)
            ZDR_stamps.append(ZDRpost)
            KDP_stamps.append(KDPpost)
            CC_stamps.append(CCpost)
            lon_stamps.append(lonpost)
            lat_stamps.append(latpost)
            postlons.append(postlon)
            postlats.append(postlat)
            ROT_stamps.append(ROTpost)
            ZDR_D_stamps.append(ZDR_D_post)
            NZDR_stamps.append(NZDRpost)
            time_stamps.append(time_start)
            azim_stamps.append(azim_post)
            VEL_stamps.append(VELpost)
        except Exception as error:
            print('storm dead or not yet formed', error)
    ...
    f.close()
    plt.show()
    SPORK_stamp = [REF_stamps, ZDR_stamps, KDP_stamps, CC_stamps, lon_stamps, lat_stamps, postlons, postlats, ROT_stamps, ZDR_D_stamps, NZDR_stamps, time_stamps, azim_stamps, VEL_stamps] 
    print('Fin')
    #export_csv = tracks_dataframe.to_csv(r'C:\Users\Nick\Downloads\tracksdataframe.csv',index=None,header=True)
    return tracks_dataframe, zdr_out_list, col_con_areas, col_con_centroid_lon, col_con_storm_lon, SPORK_stamp
